    ...
    def getLandmarks(self, frame):
        try:
            ...
            if face_landmarks is None:
                # Return a default frame and placeholder data when no face is detected
                dummy_landmarks = np.zeros((468, 2))  # MediaPipe face mesh has 468 landmarks
                dummy_eye_landmarks = np.zeros((16, 2))  # Sample size for eye landmarks
                empty_subframe = np.zeros((100, 100, 3), dtype=np.uint8)
                return np.concatenate((dummy_eye_landmarks, dummy_eye_landmarks, np.array([[1, 1]]), np.zeros((1, 2)))), True, empty_subframe
            ...
            # Create a subframe of the face region
            subframe = frame[int(y_offset):int(y_offset+y_width),int(x_offset):int(x_offset+x_width)]
            return key_points, blink, subframe
        
        except Exception as e:
            print(f"Error in getLandmarks: {e}")
            # Return a default frame and placeholder data when an error occurs
            dummy_landmarks = np.zeros((468, 2))  # MediaPipe face mesh has 468 landmarks
            dummy_eye_landmarks = np.zeros((16, 2))  # Sample size for eye landmarks
            empty_subframe = np.zeros((100, 100, 3), dtype=np.uint8)
            return np.concatenate((dummy_eye_landmarks, dummy_eye_landmarks, np.array([[1, 1]]), np.zeros((1, 2)))), True, empty_subframe
